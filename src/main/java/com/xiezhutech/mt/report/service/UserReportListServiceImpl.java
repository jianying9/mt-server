 ...
 */
@ServiceConfig(
        route = RouteNames.USER_REPORT_LIST_V1,
        requestConfigs = {
        ...
        desc = "我的检测列表查询(客户)",
        group = "检测")
public class UserReportListServiceImpl extends AbstractSessionService implements Service {
    ...
    protected void sessionExecute(Request request, Response response, SessionEntity sessionEntity) {
        ...
        if (currentUserEntity.isCustomer()) {
            //查询列表
            ...
            if (orgId != null) {
                ...
                List<String> orgIdList = currentUserEntity.getOrgIdList();
